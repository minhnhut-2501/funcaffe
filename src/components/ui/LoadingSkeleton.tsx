...
export default function LoadingSkeleton({ variant = 'table', rows = 5, cols = 1 }: Props) {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i} className="rounded-xl border border-cafe-100 p-6 skeleton-sweep">
  ...
  if (variant === 'list') {
    return (
      <div className="space-y-3 skeleton-sweep">
        {Array.from({ length: rows }).map((_, i) => (
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-cafe-100 skeleton-sweep">
