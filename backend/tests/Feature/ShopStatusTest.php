 ...
 * chốt cho từng trạng thái — xem chú thích ở ChecksShopStatus.
 */
class ShopStatusTest extends MongoTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->shop = $this->user->shops()->create(['name' => 'Quán trạng thái', 'status' => 'open']);

        $this->capGoi($this->shop);
    ...
    private function capGoi(Shop $shop): void
    {
    ...
    {
        $this->shop->update(['status' => 'closed']);
    ...
    public function test_quan_dong_cua_van_thanh_toan_duoc_ban_dang_ngoi(): void
    {
        ...
        $this->postJson("/api/shops/{$this->shop->id}/orders/{$donId}/pay", ['payment_method' => 'cash', 'cash_received' => 1_000_000])
            ->assertStatus(200);
    }
    ...
    public function test_quan_dong_cua_van_them_duoc_mon_vao_ban_dang_ngoi(): void
    {
        ...
        $this->putJson("/api/shops/{$this->shop->id}/orders/{$donId}", [
            'items' => [['product_id' => (string) $this->item->id, 'product_name_snapshot' => 'Cà phê sữa', 'quantity' => 3]],
        ])->assertStatus(200);
    }
    ...
    public function test_quan_dong_cua_van_sua_duoc_thuc_don(): void
    {
        ...
        $this->postJson("/api/shops/{$this->shop->id}/toppings", ['name' => 'Trân châu', 'price' => 5_000])
            ->assertStatus(201);
    }
    ...
    public function test_quan_ngung_hoat_dong_thi_khong_sua_duoc_thuc_don(): void
    {
        ...
        $this->postJson("/api/shops/{$this->shop->id}/products", [
            'category_id' => (string) $this->item->category_id, 'name' => 'Món mới', 'base_price' => 10_000,
        ])->assertStatus(422)->assertJsonPath('message', $thongBao);

        $this->postJson("/api/shops/{$this->shop->id}/tables", ['name' => 'Bàn 9', 'capacity' => 2])
            ->assertStatus(422)->assertJsonPath('message', $thongBao);

        $this->postJson("/api/shops/{$this->shop->id}/categories", ['name' => 'Danh mục mới'])
            ->assertStatus(422)->assertJsonPath('message', $thongBao);
    }
    ...
    public function test_quan_ngung_hoat_dong_van_xem_duoc_so_lieu_cu(): void
    {
        ...
        $this->getJson("/api/shops/{$this->shop->id}/orders?status=paid")->assertStatus(200)->assertJsonCount(1);
    ...
    public function test_mo_lai_quan_thi_ban_hang_tro_lai_binh_thuong(): void
    {
        ...
        $this->putJson("/api/shops/{$this->shop->id}", ['status' => 'open'])->assertStatus(200);
        $this->moDon()->assertStatus(201);
    }
    ...
    {
        $quanMoi = $this->postJson('/api/shops', ['name' => 'Quán thứ hai'])
            ->assertStatus(201)->json('id');

        $this->postJson("/api/shops/{$quanMoi}/tables", ['name' => 'Bàn 1', 'capacity' => 4])
            ->assertStatus(403)
            ->assertJsonPath('message', 'Quán này cần kích hoạt gói dịch vụ để sử dụng chức năng này.');

        $this->postJson("/api/shops/{$quanMoi}/products", ['name' => 'Món', 'base_price' => 10_000, 'category_id' => 'x'])
            ->assertStatus(403);
    }
    ...
    {
        $quanMoi = Shop::find($this->postJson('/api/shops', ['name' => 'Quán thứ hai'])->json('id'));
        $this->capGoi($quanMoi);

        $this->postJson("/api/shops/{$quanMoi->id}/tables", ['name' => 'Bàn 1', 'capacity' => 4])
            ->assertStatus(201);
    }
    ...
    {
        $this->deleteJson("/api/shops/{$this->shop->id}")->assertStatus(405);
        $this->assertNotNull(Shop::find((string) $this->shop->id));
    }
}
